...
for rate in injection_rates:
    with open("../runfiles/meshconfig", "a") as configfile:
        configfile.write("injection_rate =" + str(rate) + ";\n")
        matlab_file = "mesh_" + str(int(rate*100)) + ".m;\n"
        configfile.write("stats_out = " + matlab_file)
        os.system('./booksim ../runfiles/meshconfig')
"""

injection_rates = [0.01, 0.05, 0.1, 0.15]

for rate in injection_rates:
    file = "mesh_" + str(int(rate*100)) + ".m"
    outfile = open(file, "r")
